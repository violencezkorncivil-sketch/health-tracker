...
["ข้าวผัด",560,20,70,22,"1 จาน","จานเดียว",2.5,1100,6,4,0,450],
["ข้าวผัดกะเพราหมูสับไข่ดาว",680,28,72,32,"1 จาน","จานเดียว",3,1300,9,4,0,600],
["ข้าวกะเพราหมูสับ",560,25,68,20,"1 จาน","จานเดียว",3,1200,6.5,4,0,550],
["ข้าวกะเพราไก่",520,28,66,16,"1 จาน","จานเดียว",3,1200,4.5,4,0,550],
["ข้าวกะเพราหมูกรอบ",720,24,70,38,"1 จาน","จานเดียว",3,1350,13,5,0,600],
["ข้าวมันไก่",600,28,72,22,"1 จาน","จานเดียว",1.5,1184,7,5,0,500],
["ข้าวมันไก่ทอด",700,28,74,32,"1 จาน","จานเดียว",1.5,1250,10,5,0,500],
["ข้าวหมูแดง",560,24,80,16,"1 จาน","จานเดียว",2,1250,5,18,0,500],
["ข้าวหมูกรอบ",680,24,72,34,"1 จาน","จานเดียว",2,1300,12,6,0,520],
["ข้าวขาหมู",700,30,75,30,"1 จาน","จานเดียว",3,1205,11,10,0,620],
["ข้าวหมกไก่",620,26,78,22,"1 จาน","จานเดียว",3,1200,7,6,0,550],
["ข้าวคลุกกะปิ",580,18,78,20,"1 จาน","จานเดียว",4,1500,6,16,0,600],
["ข้าวไข่เจียว",520,16,60,24,"1 จาน","จานเดียว",1,700,6,1,0,300],
["ข้าวไข่ดาว",400,12,56,14,"1 จาน","จานเดียว",1,500,3.6,0,0,250],
["ข้าวหน้าเป็ด",620,28,76,22,"1 จาน","จานเดียว",2,1300,7,10,0,550],
["ข้าวผัดอเมริกัน",900,28,90,48,"1 จาน","จานเดียว",4,1900,15,12,0,800],
["ข้าวผัดปู",620,20,74,26,"1 จาน","จานเดียว",2.5,1150,6.5,4,0,480],
["ข้าวผัดกุ้ง",580,24,72,22,"1 จาน","จานเดียว",2.5,1150,5.5,4,0,480],
["ข้าวราดแกงเขียวหวานไก่",600,24,70,24,"1 จาน","จานเดียว",4,1200,15,8,0,600],
["ข้าวราดผัดกระเพราทะเล",540,26,68,18,"1 จาน","จานเดียว",3,1300,4.5,4,0,550],
["ข้าวผัดพริกแกงหมู",640,26,68,29,"1 จาน","จานเดียว",3.5,1300,10,6,0,580],
["ข้าวหน้าไก่เทอริยากิ",620,30,80,18,"1 จาน","จานเดียว",2,1400,4.5,18,0,550],
["ข้าวแกงกะหรี่ญี่ปุ่นหมูทอด",850,28,100,36,"1 จาน","จานเดียว",5,1700,13,14,0,750],
["ข้าวหน้าหมูทอด (ทงคัตสึ)",800,30,90,34,"1 จาน","จานเดียว",3,1300,11,12,0,650],
["ข้าวหน้าเนื้อ (กิวด้ง)",650,26,85,22,"1 ชาม","จานเดียว",2.5,1400,8,16,0,550],
["ผัดไทย",550,18,72,20,"1 จาน","จานเดียว",3.5,1150,4.5,18,0,500],
["ผัดไทยกุ้งสด",600,24,72,22,"1 จาน","จานเดียว",3.5,1200,5,18,0,550],
["ผัดซีอิ๊ว",560,18,70,22,"1 จาน","จานเดียว",3,1352,6,8,0,500],
["ราดหน้าหมู",520,20,66,18,"1 จาน","จานเดียว",3.5,1400,5.5,8,0,600],
["ผัดกระเพราหมูสับ (ไม่รวมข้าว)",320,24,8,20,"1 จาน","จานเดียว",2,1150,6.5,3,0,480],
["ผัดมาม่า",480,12,62,18,"1 จาน","จานเดียว",3,1500,7,5,0,350],
["สปาเก็ตตี้คาโบนาร่า",700,24,72,34,"1 จาน","จานเดียว",4,1000,16,5,0,450],
["สปาเก็ตตี้ผัดขี้เมา",560,20,70,20,"1 จาน","จานเดียว",4,1200,5,6,0,500],
["ข้าวต้มหมู",280,16,40,5,"1 ชาม","จานเดียว",1.5,800,1.8,2,0,400],
["โจ๊กหมู",300,16,42,7,"1 ชาม","จานเดียว",1.5,900,2.5,2,0,350],
["ข้าวเปล่า+ไข่ต้ม",320,10,53,7,"1 ชุด","จานเดียว",1,130,2.2,0,0,220],

// ===== ก๋วยเตี๋ยว/ซุป =====
["ก๋วยเตี๋ยวน้ำหมู",350,18,45,10,"1 ชาม","ก๋วยเตี๋ยว",2.5,1900,3.6,4,0,450],
["ก๋วยเตี๋ยวน้ำตก",400,20,45,14,"1 ชาม","ก๋วยเตี๋ยว",3,2200,5,5,0,520],
["ก๋วยเตี๋ยวต้มยำ",450,20,50,18,"1 ชาม","ก๋วยเตี๋ยว",3,2100,6,12,0,550],
["ก๋วยเตี๋ยวเรือ",380,20,44,12,"1 ชาม","ก๋วยเตี๋ยว",3,2000,4.3,6,0,500],
["ก๋วยเตี๋ยวแห้ง",420,20,50,16,"1 ชาม","ก๋วยเตี๋ยว",2.5,1300,5.5,5,0,470],
["เย็นตาโฟ",400,18,55,12,"1 ชาม","ก๋วยเตี๋ยว",4,2500,4,14,0,550],
["ก๋วยเตี๋ยวเป็ดตุ๋น",450,24,50,16,"1 ชาม","ก๋วยเตี๋ยว",2.5,1950,5.5,6,0,520],
["บะหมี่หมูแดง",420,20,55,13,"1 ชาม","ก๋วยเตี๋ยว",2.5,1480,4.5,9,0,430],
["บะหมี่เกี๊ยวกุ้ง",400,20,52,12,"1 ชาม","ก๋วยเตี๋ยว",2.5,1700,4,5,0,420],
["ก๋วยจั๊บน้ำข้น",480,18,50,22,"1 ชาม","ก๋วยเตี๋ยว",2,1900,8,3,0,480],
["ราเมง",550,24,65,20,"1 ชาม","ก๋วยเตี๋ยว",4,2300,7,6,0,600],
["ก๋วยเตี๋ยวไก่มะระ",320,20,38,8,"1 ชาม","ก๋วยเตี๋ยว",3,1700,2.4,3,0,500],
["สุกี้น้ำ",320,22,30,10,"1 ชาม","ก๋วยเตี๋ยว",4,1400,3.5,8,0,700],
["สุกี้แห้ง",450,24,40,20,"1 จาน","ก๋วยเตี๋ยว",4,1400,6.5,9,0,700],
["มาม่าต้มยำ (ซอง)",320,7,45,12,"1 ซอง","ก๋วยเตี๋ยว",2,1500,6,5,0,180],
["ต้มยำกุ้งน้ำใส",180,20,10,6,"1 ถ้วย","ก๋วยเตี๋ยว",2,1200,1.8,5,0,520],
["ต้มยำกุ้งน้ำข้น",320,20,14,20,"1 ถ้วย","ก๋วยเตี๋ยว",2,1300,11,6,0,550],
["ต้มจืดหมูสับ",150,14,8,6,"1 ถ้วย","ก๋วยเตี๋ยว",1.5,800,2.2,2,0,400],
["แกงจืดเต้าหู้หมูสับ",160,14,9,7,"1 ถ้วย","ก๋วยเตี๋ยว",1.5,850,2.4,2,0,420],

// ===== กับข้าว/แกง =====
["แกงเขียวหวานไก่",300,18,12,20,"1 ถ้วย","กับข้าว",3,1000,13,6,0,480],
["แกงเผ็ดหมู",320,18,12,22,"1 ถ้วย","กับข้าว",3,1000,14,6,0,480],
["แกงส้มผักรวมกุ้ง",180,16,14,6,"1 ถ้วย","กับข้าว",3.5,1130,1.2,8,0,550],
["แกงมัสมั่นไก่",380,20,20,25,"1 ถ้วย","กับข้าว",3,1000,15,10,0,550],
["พะแนงหมู",350,20,10,26,"1 ถ้วย","กับข้าว",2,1000,16,5,0,450],
["ต้มข่าไก่",280,16,10,20,"1 ถ้วย","กับข้าว",1.5,1000,13,4,0,450],
["ผัดผักบุ้งไฟแดง",120,4,10,8,"1 จาน","กับข้าว",3,800,1.6,3,0,400],
["ผัดผักรวม",130,5,12,8,"1 จาน","กับข้าว",3.5,700,1.6,4,0,420],
["ผัดคะน้าหมูกรอบ",320,14,12,25,"1 จาน","กับข้าว",3,1050,8.5,3,0,450],
["ไข่เจียว",220,12,2,18,"1 ฟอง (จาน)","กับข้าว",0,400,4.5,0,0,160],
["ไข่ต้ม",78,6.3,0.6,5.3,"1 ฟอง","กับข้าว",0,62,1.6,0.6,0],   // ค่าละเอียดจาก USDA 173424
["ไข่ตุ๋น",130,10,3,8,"1 ถ้วย","กับข้าว",0.2,500,2.5,1,0,180],
["หมูทอดกระเทียม",380,26,6,28,"1 จาน","กับข้าว",0.5,750,9.5,2,0,450],
["ไก่ทอด",300,24,12,18,"1 ชิ้นใหญ่","กับข้าว",0.5,650,4.5,1,0,320],
["ไก่ย่าง",240,30,3,12,"1/4 ตัว","กับข้าว",0.2,600,3.4,2,0,400],
["ปลาทอด",280,28,4,17,"1 ตัวกลาง","กับข้าว",0,500,3.5,0,0,450],
["ปลานึ่งมะนาว",220,32,6,8,"1 จาน","กับข้าว",1,1000,2,3,0,600],
["ปลาเผา",250,35,2,11,"1 ตัว","กับข้าว",0,400,2.5,0,0,620],
["หมูปิ้ง",90,7,4,5,"1 ไม้","กับข้าว",0,270,2.4,3,0],   // คำนวณจากส่วนผสม: หมู 35 ก. + น้ำตาล/นมข้น/กะทิ + น้ำปลา-ซีอิ๊ว
["ไก่ปิ้ง",80,9,2,4,"1 ไม้","กับข้าว",0,220,1.2,1.5,0],   // คำนวณจากส่วนผสม: ไก่ 35 ก. + น้ำตาลหมัก + ซีอิ๊ว
["ลูกชิ้นปิ้ง",120,6,10,6,"1 ไม้ (5 ลูก)","กับข้าว",0.3,700,2.4,4,0,120],
["ไส้กรอกอีสาน",180,8,10,12,"1 ไม้","กับข้าว",0.5,800,4.5,2,0,180],
["ส้มตำไทย",120,4,20,3,"1 จาน","กับข้าว",4,900,0.6,14,0,450],
["ส้มตำปูปลาร้า",150,8,18,5,"1 จาน","กับข้าว",4,1300,1,12,0,500],
["ลาบหมู",280,24,8,17,"1 จาน","กับข้าว",2,950,6,2,0,480],
["น้ำตกหมู",300,26,8,18,"1 จาน","กับข้าว",2,950,6.5,3,0,500],
["ยำวุ้นเส้น",250,14,28,9,"1 จาน","กับข้าว",2,1000,2.5,8,0,400],
["ยำทะเล",220,26,14,6,"1 จาน","กับข้าว",2,1100,1.2,8,0,550],
["สลัดผัก (น้ำสลัดใส)",120,4,12,6,"1 จาน","กับข้าว",3,400,1,7,0,400],
["สลัดอกไก่",280,30,12,12,"1 จาน","กับข้าว",3,700,2.5,6,0,650],
["เต้าหู้ทอด",180,12,6,12,"1 จานเล็ก","กับข้าว",1.5,350,1.8,1,0,250],
["ปอเปี๊ยะทอด",180,5,20,9,"2 ชิ้น","กับข้าว",1.5,350,3,3,0,150],
["หมูสามชั้นทอด",420,20,2,38,"100 กรัม","วัตถุดิบ",0,300,14,0,0,280],
["กุ้งเผา",120,24,1,2,"4 ตัว","กับข้าว",0,300,0.4,0,0,300],
["หมูกระทะ (ต่อมื้อ)",900,50,40,60,"1 มื้อ","กับข้าว",5,2500,21,14,0,1100],
["ชาบู (ต่อมื้อ)",700,45,45,35,"1 มื้อ","กับข้าว",6,2200,13,12,0,1200],
["ปิ้งย่างเกาหลี (ต่อมื้อ)",950,55,45,62,"1 มื้อ","กับข้าว",5,2500,22,18,0,1200],

// ===== ฟาสต์ฟู้ด/ขนมปัง =====
["แฮมเบอร์เกอร์",550,25,45,30,"1 ชิ้น","ฟาสต์ฟู้ด",3,900,11,8,0,400],
["เฟรนช์ฟรายส์",340,4,44,17,"1 ห่อกลาง","ฟาสต์ฟู้ด",4,350,2.6,0,0,750],
["พิซซ่า",280,12,32,11,"1 ชิ้น","ฟาสต์ฟู้ด",2,640,5,4,0,200],
["ไก่ทอด KFC",290,20,12,18,"1 ชิ้น","ฟาสต์ฟู้ด",0.5,800,4,0,0,250],
["แซนด์วิชแฮมชีส",320,15,32,14,"1 ชิ้น","ฟาสต์ฟู้ด",2,800,6,4,0,250],
["ขนมปังปิ้งเนยน้ำตาล",250,5,32,11,"1 แผ่น","ฟาสต์ฟู้ด",1.5,300,6,12,0,80],
["ครัวซองต์",270,6,30,14,"1 ชิ้น","ฟาสต์ฟู้ด",1.5,320,8,6,0,80],
["โดนัท",250,4,32,12,"1 ชิ้น","ฟาสต์ฟู้ด",1,250,5.5,14,0,70],
["ซาลาเปาไส้หมูสับ",220,9,32,6,"1 ลูก","ฟาสต์ฟู้ด",1,400,2,6,0,150],
["ปาท่องโก๋",90,2,10,5,"1 คู่","ฟาสต์ฟู้ด",0.4,150,2.2,0,0,30],
["ข้าวเหนียวหมูปิ้ง",370,12,60,9,"1 ชุด","ฟาสต์ฟู้ด",1.5,700,3,8,0,220],
["ข้าวเหนียวไก่ทอด",520,22,62,20,"1 ชุด","ฟาสต์ฟู้ด",1.5,800,5,3,0,320],

// ===== ผลไม้ =====
["กล้วยน้ำว้า 1 ผล",100,1.2,25,0.3,"1 ผล","ผลไม้",2.6,1,0.1,0,0,350],
["แอปเปิ้ล",80,0.4,21,0.3,"1 ผล","ผลไม้",3.6,1,0.1,0,0,160],
["ส้ม",60,1,15,0.2,"1 ผล","ผลไม้",3,0,0,0,0,230],
["มะม่วงสุก 1 ผล",150,1,38,0.5,"1 ผล","ผลไม้",4,3,0.1,0,0,350],
["แตงโม",50,1,12,0.2,"1 จานเล็ก","ผลไม้",0.6,2,0,0,0,170],
["สับปะรด",60,0.6,15,0.1,"1 จานเล็ก","ผลไม้",1.6,2,0,0,0,150],
["องุ่น",70,0.7,18,0.2,"15 ผล","ผลไม้",0.9,2,0.1,0,0,190],

// ===== ขนม/ของหวาน =====
["ไอศกรีม",200,4,24,10,"1 ถ้วย","ของหวาน",0.5,80,6,22,0,200],
["เค้กช็อกโกแลต",350,5,45,17,"1 ชิ้น","ของหวาน",2,300,9,33,0,180],
["บิงซู",450,8,70,15,"1 ถ้วย","ของหวาน",2,150,9,55,0,350],
["ข้าวเหนียวมะม่วง",480,6,85,13,"1 จาน","ของหวาน",3,200,10,45,0,350],
["บัวลอยน้ำขิง",280,4,52,7,"1 ถ้วย","ของหวาน",1.5,60,5.5,32,0,150],
["ทองหยิบ/ฝอยทอง",150,3,30,3,"3 ชิ้น","ของหวาน",0,30,1,28,0,40],
["มันฝรั่งทอดกรอบ (ถุง)",270,3,28,16,"1 ถุงเล็ก","ของหวาน",2.5,350,5,1,0,550],
["ช็อกโกแลตแท่ง",250,3,28,14,"1 แท่ง","ของหวาน",2,40,8.5,25,0,200],
["คุกกี้",120,1.5,16,6,"2 ชิ้น","ของหวาน",0.5,90,3,8,0,40],
["ถั่วอบ",180,7,7,15,"1 กำมือ (30 ก.)","ของหวาน",3,120,2,1,0,220],

// ===== เครื่องดื่ม =====
["น้ำเปล่า",0,0,0,0,"1 แก้ว","เครื่องดื่ม",0,2,0,0,0,1],
["ชาเขียวไม่หวาน",0,0,0,0,"1 แก้ว","เครื่องดื่ม",0,3,0,0,0,25],
["กาแฟดำ/อเมริกาโน่ (ไม่ใส่น้ำตาล)",5,0.3,0,0,"1 แก้ว","เครื่องดื่ม",0,5,0,0,0,115],
["กาแฟเย็น (หวานปกติ)",250,4,38,9,"1 แก้ว","เครื่องดื่ม",0,100,6,34,0,200],
["ลาเต้ร้อน",150,8,14,7,"1 แก้ว","เครื่องดื่ม",0,105,4.5,0,0,350],
["ชาไทยเย็น",280,4,45,10,"1 แก้ว","เครื่องดื่ม",0,120,7,42,0,180],
["ชานมไข่มุก",400,6,70,10,"1 แก้ว","เครื่องดื่ม",0.5,150,7,60,0,200],
["น้ำอัดลม",150,0,38,0,"1 กระป๋อง","เครื่องดื่ม",0,45,0,38,0,10],
["น้ำผลไม้กล่อง",120,0.5,29,0,"1 กล่อง","เครื่องดื่ม",0.3,15,0,27,0,250],
["นมสด",150,8,12,8,"1 กล่อง 200 มล.","เครื่องดื่ม",0,100,5,0,0,320],
["นมถั่วเหลือง",120,6,15,4,"1 กล่อง","เครื่องดื่ม",1,100,0.6,9,0,250],
["โยเกิร์ตรสธรรมชาติ",100,6,12,3,"1 ถ้วย","เครื่องดื่ม",0,80,1.9,0,0,280],
["เวย์โปรตีน 1 สคูป",120,24,3,1.5,"1 สคูป","เครื่องดื่ม",0.5,100,0.8,1,0,150],
...
["เหล้า Regency (ฝา 30 มล.)",59,0,0,0,"1 ฝา 30 มล.","เครื่องดื่ม",0,0,0,0,8.3],
["Regency + โซดา (1 ฝา)",59,0,0,0,"1 แก้ว (1 ฝา)","เครื่องดื่ม",0,10,0,0,8.3],
["เหล้า Regency (ขวดเล็ก 350 มล.)",687,0,0,0,"1 ขวด 350 มล.","เครื่องดื่ม",0,0,0,0,96.7],
["เหล้า Regency (ขวดกลม 500 มล.)",981,0,0,0,"1 ขวด 500 มล.","เครื่องดื่ม",0,0,0,0,138.1],
["เหล้า Regency (ขวดใหญ่ 700 มล.)",1374,0,0,0,"1 ขวด 700 มล.","เครื่องดื่ม",0,0,0,0,193.3],
["เหล้า 28 ดีกรี (ฝา 30 มล.)",47,0,0,0,"1 ฝา 30 มล.","เครื่องดื่ม",0,0,0,0,6.6],
["เหล้า 35 ดีกรี (ฝา 30 มล.)",59,0,0,0,"1 ฝา 30 มล.","เครื่องดื่ม",0,0,0,0,8.3],
["เหล้า 40 ดีกรี (ฝา 30 มล.)",67,0,0,0,"1 ฝา 30 มล.","เครื่องดื่ม",0,0,0,0,9.5],
["เหล้าขาว 40 ดีกรี (ฝา 30 มล.)",67,0,0,0,"1 ฝา 30 มล.","เครื่องดื่ม",0,0,0,0,9.5],
["โซดา",0,0,0,0,"1 แก้ว","เครื่องดื่ม",0,15,0,0,0],
["ไวน์แดง",126,0.1,3.9,0.0,"1 แก้ว 150 มล.","เครื่องดื่ม",0.0,6,0.0,0.9,15.7,188],   // Alcoholic beverage, wine, table, red,
["น้ำหวาน/น้ำแดง",120,0,30,0,"1 แก้ว","เครื่องดื่ม",0,30,0,30,0,5],
// ===== วัตถุดิบเดี่ยว / อาหารคลีน (กินแยกจาน) =====
["สะโพกไก่ย่าง (มีหนัง)",230,24,0,15,"100 กรัม","วัตถุดิบ",0,90,4.2,0,0,240],
["น่องไก่ต้ม",170,24,0,8,"1 น่อง","วัตถุดิบ",0,90,2.2,0,0,230],
["ปีกไก่ทอด",160,12,4,11,"1 ชิ้น","วัตถุดิบ",0.1,300,3,0,0,110],
...
["ปลาทูนึ่ง",150,22,0,7,"1 ตัว","วัตถุดิบ",0,260,2.2,0,0,300],
...
["ไข่ขาวต้ม",17,3.6,0.2,0,"1 ฟอง","วัตถุดิบ",0,55,0,0,0,54],
["ไข่ต้ม 2 ฟอง",156,12,1.2,10,"2 ฟอง","วัตถุดิบ",0,130,3.2,0,0,130],
["ไข่คน (ใส่น้ำมันนิด)",180,12,1.5,14,"2 ฟอง","วัตถุดิบ",0,180,4,0,0,150],
["เต้าหู้ขาวอ่อน",70,7,2,4,"1 หลอด","วัตถุดิบ",0.5,15,0.6,0,0,130],
...
["บล็อคโคลี่ผัดน้ำมัน",100,3,8,7,"1 จานเล็ก","วัตถุดิบ",3,300,1.2,0,0,320],
...
["ผักรวมลวก",40,2.5,7,0.4,"1 จาน","วัตถุดิบ",3,30,0.1,0,0,270],
["ผักสลัดสด (ไม่ใส่น้ำสลัด)",20,1.5,3,0.2,"1 จาน","วัตถุดิบ",1.5,15,0,0,0,200],
["มะเขือเทศ",20,1,4,0.2,"1 ลูก","วัตถุดิบ",1.2,5,0,0,0,240],
["แตงกวา",15,0.7,3,0.1,"1 ลูก","วัตถุดิบ",0.6,2,0,0,0,150],
["ฟักทองนึ่ง",50,1.2,12,0.1,"100 กรัม","วัตถุดิบ",1.5,2,0,0,0,340],
["เห็ดลวก",25,3,4,0.3,"100 กรัม","วัตถุดิบ",1.5,6,0,0,0,330],
["ข้าวไรซ์เบอร์รี่",230,5,47,1.5,"1 ทัพพี","วัตถุดิบ",3,5,0.3,0,0,160],
["มันหวานนึ่ง",90,1.6,21,0.1,"100 กรัม","วัตถุดิบ",3.3,36,0,0,0,340],
["ข้าวโพดต้ม",90,3,19,1.2,"1 ฝัก","วัตถุดิบ",2.4,15,0.2,0,0,250],
["วุ้นเส้นลวก",80,0.1,20,0,"1 ถ้วย","วัตถุดิบ",0.4,5,0,0,0,10],
["เส้นก๋วยเตี๋ยวลวก",190,3,42,0.5,"1 ชาม","วัตถุดิบ",1.5,20,0.1,0,0,40],
["เนยถั่ว",95,4,3,8,"1 ช้อนโต๊ะ","วัตถุดิบ",0.9,75,1.6,1.5,0,105],
["น้ำมันพืช (ผัด 1 ช้อน)",120,0,0,14,"1 ช้อนโต๊ะ","วัตถุดิบ",0,0,2,0,0,0],
...
["ชีสแผ่น",60,4,1,5,"1 แผ่น","วัตถุดิบ",0,270,3,0,0,25],
["น้ำพริกกะปิ",60,3,5,3,"1 ช้อนโต๊ะ","วัตถุดิบ",1,900,0.8,2,0,120],
["น้ำจิ้มไก่",35,0,9,0,"1 ช้อนโต๊ะ","วัตถุดิบ",0.1,300,0,7,0,20],
["น้ำสลัดครีม",90,0.5,3,8,"1 ช้อนโต๊ะ","วัตถุดิบ",0,200,1.3,2.5,0,15],
["น้ำปลาหวาน",50,0.5,12,0,"1 ช้อนโต๊ะ","วัตถุดิบ",0.1,900,0,11,0,30],
["ซุปใสผัก",40,2,6,0.5,"1 ถ้วย","วัตถุดิบ",1.5,700,0.1,1,0,250],
["โปรตีนบาร์",200,15,20,7,"1 แท่ง","วัตถุดิบ",3,200,3,8,0,200],
["เวย์โปรตีน 2 สคูป",240,48,6,3,"2 สคูป","วัตถุดิบ",1,200,1.5,2,0,300],
...
["เบคอนทอดสุก",88,5.7,0.2,6.9,"2 แผ่น","วัตถุดิบ",0.0,351,2.3,0.0,0,87],   // Pork, cured, bacon, cooked, baked
["แฮมหมูอบ",46,6.2,0.1,2.1,"1 แผ่น","วัตถุดิบ",0.0,388,0.7,0.0,0,101],   // Pork, cured, ham, boneless, extra lean and regular, roasted
...
["เนื้อวัวบดปิ้งเป็นแพตตี้",212,22.0,0.0,13.1,"1 ชิ้น","วัตถุดิบ",0.0,61,5.0,0.0,0,270],   // Beef, ground, 85% lean meat / 15% fat, patty, cooked, broiled
...
["ไข่ไก่ดิบ",72,6.3,0.4,4.8,"1 ฟอง","วัตถุดิบ",0.0,71,1.6,undefined,0,69],   // Egg, whole, raw, fresh
["ไข่ดาว (ทอดน้ำมัน)",90,6.3,0.4,6.8,"1 ฟอง","วัตถุดิบ",0.0,95,2.0,undefined,0,70],   // Egg, whole, cooked, fried
...
["ถั่วลูกไก่ (ชิกพี) ต้มสุก",269,14.5,45.0,4.2,"1 ถ้วย 164 ก.","วัตถุดิบ",12.5,11,0.4,undefined,0],   // Chickpeas (garbanzo beans, bengal gram), mature seeds, cooked, boiled,
...
["เกลือแกง",0,0.0,0.0,0.0,"1 ช้อนชา 6 ก.","วัตถุดิบ",0.0,2325,0.0,undefined,0],   // Salt, table
...
["ผักคะน้าสด",31,2.7,5.4,0.5,"100 ก.","วัตถุดิบ",1.6,undefined,undefined,undefined,0],   // 04111 ผักคะน้า
["ผักกวางตุ้ง",11,1.5,1.6,0.2,"100 ก.","วัตถุดิบ",1.6,undefined,undefined,undefined,0],   // 04012 กวางตุ้ง, ต้นและใบ
["ผักกาดขาว",11,1.5,1.6,0.1,"100 ก.","วัตถุดิบ",0.5,undefined,undefined,undefined,0],   // 04099 ผักกาดขาว
["ผักกาดเขียว",22,2.1,3.4,0.2,"100 ก.","วัตถุดิบ",0.7,undefined,undefined,undefined,0],   // 04100 ผักกาดเขียว
["ปวยเล้ง",25,2.6,2.3,0.9,"100 ก.","วัตถุดิบ",0.7,undefined,undefined,undefined,0],   // 04095 ปวยเล้ง
["กะหล่ำปลีสด",14,1.6,2.2,0.1,"100 ก.","วัตถุดิบ",1.2,undefined,undefined,undefined,0],   // 04019 กะหล่าปลี
...
["ผักชี",24,2.7,3.4,0.6,"100 ก.","วัตถุดิบ",3.0,undefined,undefined,undefined,0],   // 04112 ผักซี
...
["พริกชี้ฟ้าเขียว",11,0.3,2.6,0.1,"20 ก.","วัตถุดิบ",0.4,undefined,undefined,undefined,0],   // 04143 พริกชี้ฟ้า, เขียว
["พริกหยวก",23,1.1,5.0,0.2,"100 ก.","วัตถุดิบ",1.7,undefined,undefined,undefined,0],   // 04146 พริกหยวก
["ถั่วพู",21,2.3,3.8,0.2,"100 ก.","วัตถุดิบ",2.9,6,undefined,undefined,0,307],   // 04072 ถั่วพู
["ถั่วฝักยาวสด",35,2.8,6.6,0.3,"100 ก.","วัตถุดิบ",1.3,undefined,undefined,undefined,0],   // 04071 ถั่วฝักยาว, แดง
["ถั่วลันเตาฝัก",58,4.3,11.3,0.1,"100 ก.","วัตถุดิบ",1.4,undefined,undefined,undefined,0],   // 04074 ถั่วลันเตา
...
["บวบเหลี่ยม",18,0.7,3.6,0.2,"100 ก.","วัตถุดิบ",0.3,undefined,undefined,undefined,0],   // 04085 บวบเหลี่ยม
["น้ำเต้า",17,0.6,3.7,0.1,"100 ก.","วัตถุดิบ",0.3,undefined,undefined,undefined,0],   // 04080 น้้าเต้า, ผล
["เนื้อฟักทองสด",75,1.9,15.8,0.9,"100 ก.","วัตถุดิบ",2.0,undefined,undefined,undefined,0],   // 04154 ฟักทอง, เนื้อ
["ยอดฟักทอง",19,2.0,3.2,0.2,"100 ก.","วัตถุดิบ",0.8,undefined,undefined,undefined,0],   // 04156 ฟักทอง, ยอดอ่อน
["มะระขี้นก",20,1.2,4.6,0.4,"100 ก.","วัตถุดิบ",3.8,26,undefined,undefined,0],   // 04185 มะระขึ้นก
...
["มะเขือเทศลูกเล็ก",24,1.1,4.7,0.3,"100 ก.","วัตถุดิบ",1.1,undefined,undefined,undefined,0],   // 04171 มะเขือเทศ, ลูกเล็ก
["มะเขือเทศลูกใหญ่",15,0.6,3.3,0.2,"100 ก.","วัตถุดิบ",1.1,undefined,undefined,undefined,0,156],   // 04172 มะเขือเทศ, ลูกใหญ่
["มะละกอดิบ (ส้มตำ)",37,0.8,8.9,0.1,"100 ก.","วัตถุดิบ",0.8,undefined,undefined,undefined,0],   // 04188 มะละกอดิบ, ผลป้อม
...
["หน่อไม้ไผ่ป่า",33,2.4,6.4,0.2,"100 ก.","วัตถุดิบ",1.0,undefined,undefined,undefined,0],   // 04222 หน่อไม้ไผ่ป่า
["หน่อไม้ต้ม",22,1.8,3.7,0.6,"100 ก.","วัตถุดิบ",2.3,undefined,undefined,undefined,0],   // 04223 หน่อไม้พันธุ์กิมซุง, ต้ม
["หน่อไม้หวาน",21,1.5,3.9,0.3,"100 ก.","วัตถุดิบ",0.8,undefined,undefined,undefined,0],   // 04226 หน่อไม้หวาน, หน่อใหญ่
...
["ยอดมะระ",87,5.8,17.1,0.3,"100 ก.","วัตถุดิบ",1.9,undefined,undefined,undefined,0],   // 04184 มะระ, ยอดอ่อน
...
["เห็ดนางฟ้า",35,2.3,6.4,0.3,"100 ก.","วัตถุดิบ",0.7,undefined,undefined,undefined,0],   // 04241 เห็ดนางฟ้า
["เห็ดนางรม",30,2.1,5.3,0.3,"100 ก.","วัตถุดิบ",0.5,undefined,undefined,undefined,0],   // 04242 เห็ดนางรม
["เห็ดฟาง",35,3.2,5.9,0.2,"100 ก.","วัตถุดิบ",0.9,undefined,undefined,undefined,0],   // 04243 เห็ดบัว (เห็ดฟาง)
...
["มันแกว",35,0.9,8.2,0.1,"100 ก.","วัตถุดิบ",1.4,undefined,undefined,undefined,0],   // 02005 มันแกว,ดิบ
["มันเทศเนื้อม่วง",102,0.8,25.5,0.2,"100 ก.","วัตถุดิบ",2.4,21,undefined,undefined,0,250],   // 02011 มันเทศ, เนื้อสีม่วงแดง, ดิบ
["หัวไชเท้า",22,0.8,5.5,0.0,"100 ก.","วัตถุดิบ",0.7,undefined,undefined,undefined,0],   // 02021 หัวผักกาด,หัวไซชเท้า, ดิบ
// --- ผลไม้ ---
["กล้วยน้ำว้า 100 ก.",143,1.1,35.4,0.2,"100 ก.","ผลไม้",2.3,undefined,undefined,undefined,0],   // 05015 กล้วยน้าว้า, สุก
["กล้วยหอม 100 ก.",128,0.9,31.7,0.2,"100 ก.","ผลไม้",1.9,undefined,undefined,undefined,0],   // 05021 กล้วยหอม, สุก
...
["มังคุด",79,0.5,20.1,0.0,"100 ก.","ผลไม้",1.7,undefined,undefined,undefined,0],   // 05110 มังคุด
["เงาะโรงเรียน",73,1.0,17.4,0.3,"100 ก.","ผลไม้",1.6,undefined,undefined,undefined,0,161],   // 05036 เงาะ,โรงเรียน
["ลางสาด",67,0.9,15.9,0.1,"100 ก.","ผลไม้",0.3,undefined,undefined,undefined,0],   // 05116 ลางสาด
["ลำไยกะโหลก",77,0.9,18.4,0.1,"100 ก.","ผลไม้",0.2,undefined,undefined,undefined,0],   // 05118 ลําใย, กะโหลก
["ชมพู่",26,0.5,6.6,0.0,"100 ก.","ผลไม้",1.1,undefined,undefined,undefined,0],   // 05044 ชมพู่,เมืองเพชร
["น้อยหน่า 100 ก.",92,1.4,22.6,0.2,"100 ก.","ผลไม้",2.7,undefined,undefined,undefined,0],   // 05065 น้อยหน่า
["พุทราไทย",89,1.7,19.7,0.8,"100 ก.","ผลไม้",1.0,undefined,undefined,undefined,0],   // 05073 พุทราไทย
...
["แตงไทยสุก",13,0.2,3.4,0.0,"100 ก.","ผลไม้",0.3,undefined,undefined,undefined,0],   // 05050 แตงไทย,สุก
["แตงโมกินรี",33,0.7,7.5,0.1,"100 ก.","ผลไม้",0.4,undefined,undefined,undefined,0,122],   // 05051 แตงโม,กินรี
["ฝรั่งแป้นสีทอง",37,0.5,9.9,0.2,"100 ก.","ผลไม้",3.3,4,undefined,undefined,0],   // 05069 ฝรั่ง, แป้นสีทอง
["มะม่วงคาราบาวสุก",61,0.5,14.0,0.6,"100 ก.","ผลไม้",1.0,undefined,undefined,undefined,0],   // 05091 มะม่วง, คาราบาว, สุก
["มะม่วงดิบ (แก้ว)",71,0.5,18.1,0.2,"100 ก.","ผลไม้",2.4,undefined,undefined,undefined,0],   // 05086 มะม่วง, แก้ว, ดิบ
...
// --- วัตถุดิบ ---
["ข้าวเหนียวนึ่ง",138,2.5,31.4,0.4,"1 ทัพพี 60 ก.","วัตถุดิบ",0.3,undefined,undefined,undefined,0],   // 01039 ข้าวเหนียว,นึ่ง
["ข้าวกล้องหอมมะลิ (ข้าวสาร)",357,7.7,75.2,3.5,"100 ก.","วัตถุดิบ",3.3,20,undefined,undefined,0],   // 01011 ข้าวกล้อง, หอมมะลิ, ดิบ
["ข้าวสังข์หยด (ข้าวสาร)",359,9.0,73.1,4.2,"100 ก.","วัตถุดิบ",3.8,undefined,undefined,undefined,0],   // 01023 ข้าวเจ้า, สังข์หยด, ดิบ
["ข้าวเหนียวดำ (ข้าวสาร)",354,8.2,76.1,3.0,"100 ก.","วัตถุดิบ",4.9,undefined,undefined,undefined,0],   // 01037 ข้าวเหนียวดํา, ดิบ
["ก๋วยเตี๋ยวเส้นเล็กสด",220,4.4,49.4,0.6,"1 ก้อน 100 ก.","วัตถุดิบ",0.2,undefined,undefined,undefined,0],   // 01002 ก๋วยเตี๋ยว, เส้นเล็ก, เส้นสด
["ก๋วยเตี๋ยวเส้นใหญ่สด",135,2.5,29.6,0.8,"1 ก้อน 100 ก.","วัตถุดิบ",0.1,undefined,undefined,undefined,0],   // 01003 ก๋วยเตี๋ยว, เส้นใหญ่, เส้นสด
["ขนมจีนแป้งสด",54,0.8,12.7,0.0,"1 จับ 60 ก.","วัตถุดิบ",0.1,undefined,undefined,undefined,0],   // 01004 ขนมจีน, แป้งสด
["ขนมจีนแป้งหมัก",46,0.5,11.0,0.1,"1 จับ 60 ก.","วัตถุดิบ",0.1,undefined,undefined,undefined,0],   // 01005 ขนมจีน, แป้งหมัก
["ขนมปังปอนด์",99,3.7,18.9,1.0,"1 แผ่น 30 ก.","วัตถุดิบ",0.1,undefined,undefined,undefined,0],   // 01006 ขนมปังปอนด์
["เต้าหู้เหลือง",150,13.5,9.1,6.7,"100 ก.","วัตถุดิบ",0.3,undefined,undefined,undefined,0],   // 03015 เต้าหู้เหลือง
["ถั่วลิสงอบเกลือ",181,8.3,6.2,14.3,"1 กำมือ 30 ก.","วัตถุดิบ",3.1,undefined,undefined,undefined,0],   // 03039 ถั่วลิสง,อบเกลือ, รวมเยื่อหุ้มเมล็ด
["หมูเนื้อแดงดิบ",110,19.6,0.4,3.3,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06050 หมู, เนื้อ, ดิบ
["ตับหมูดิบ",117,19.8,2.6,3.0,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06054 หมู,ตับ, ดิบ
["มันหมูดิบ",714,4.7,2.7,76.0,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06057 หมู,มัน, ดิบ
["เนื้อไก่รวมดิบ",165,19.5,0.0,9.7,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06005 ไก่, เนื้อ, ดิบ
["ไก่บ้านเนื้อล้วนดิบ",111,20.8,0.0,3.1,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06012 ไก่บ้าน,กิ้น, ดิบ
["สะโพกไก่บ้านมีหนัง",124,21.1,0.0,4.4,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06015 ไก่บ้าน, สะโพก, เนื้อและหนัง, ดิบ
["แคบหมูมีมัน",188,14.7,0.0,14.3,"30 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06023 แคบหมู,มีมัน
["แคบหมูไม่มีมัน",154,16.9,3.4,8.1,"30 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06024 แคบหมู,ไม่มีมัน
["ไส้กรอกค็อกเทลหมู",242,11.8,7.2,18.4,"100 ก.","วัตถุดิบ",0.0,712,undefined,undefined,0,151],   // 06044 ไส้กรอกคอกเทล, หมู
["ไส้กรอกเวียนนาหมู",263,14.5,2.1,21.8,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06046 ไส้กรอกเวียนนา, หมู
["ไส้อั่ว",420,18.0,5.7,36.1,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 06048 ไส้อั้ว, ดิบ
...
["ปลาดุกสด",114,23.0,0.0,2.4,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07027 ปลาดุก, สด
["ปลาทูสด",140,20.0,0.0,6.7,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07035 ปลาทู,สด
["ปลาทูนึ่ง 1 ตัว",82,14.9,0.0,2.4,"1 ตัว 60 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07034 ปลาทู,นึ่ง
["ปลาสลิดสด",76,17.2,0.0,0.8,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07059 ปลาสลิด, สด
["ปลานิลสด",93,18.2,0.0,2.2,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07041 ปลานิล, สด
["ปลาทับทิมสด",113,21.0,0.7,2.9,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07039 ปลาทับทิม, สด
...
["ปลาไหลสด",87,18.9,0.1,1.2,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07074 ปลาไหล, สด
["ปลาทูน่าสด",110,24.8,0.0,1.2,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07036 ปลาทูน่า, สด
["ปลาโอสด",109,24.5,0.2,1.1,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0,876],   // 07078 ปลาโอ, สด
["ปลาหมึกกล้วยสด",62,13.7,0.4,0.6,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07070 ปลาหมึกกระเป๋า, สด
["กุ้งกุลาดำสด",120,14.3,5.7,4.4,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07003 กุ้งกุลาดํา,หัว, สด
...
["หอยแครงสด",81,12.9,4.3,1.3,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07094 หอยแครง, สด
["หอยแมลงภู่สด",53,8.1,3.1,0.9,"100 ก.","วัตถุดิบ",0.0,undefined,undefined,undefined,0],   // 07098 หอยแมลงภู่, สด
...
["นมยูเอชทีรสจืด",133,6.8,8.4,8.0,"1 กล่อง 200 มล.","เครื่องดื่ม",0.0,undefined,undefined,0.0,0],   // 09003 นม, ยูเอชที, รสธรรมชาติ
["นมยูเอชทีรสหวาน",141,6.8,11.3,7.8,"1 กล่อง 200 มล.","เครื่องดื่ม",0.0,undefined,undefined,undefined,0],   // 09004 นม, ย ูเอชที, รสหวาน
["นมแพะดิบ",198,9.8,10.4,13.2,"1 แก้ว 240 มล.","เครื่องดื่ม",0.0,208,undefined,0.0,0,460],   // 09028 นมแพะ, ดิบ
["โยเกิร์ตจืด 1 ถ้วย",160,5.7,20.9,6.0,"1 ถ้วย 150 ก.","เครื่องดื่ม",0.0,undefined,undefined,0.0,0],   // 09035 โยเกิร์ต, รสธรรมชาติ
["โยเกิร์ตไขมันต่ำรสผลไม้",138,4.9,26.8,1.2,"1 ถ้วย 150 ก.","เครื่องดื่ม",0.0,undefined,undefined,undefined,0],   // 09034 โยเกิร์ต, ไขมันต่า, รสผลไม้รวม
// --- จานเดียว ---
["ส้มตำอีสาน",56,3.6,10.8,0.6,"1 จาน 200 ก.","จานเดียว",1.8,undefined,undefined,undefined,0],   // 11204 สัมตํา, อีสาน
// --- กับข้าว ---
["ลาบเนื้อ (รวมผักสด)",168,18.6,11.0,6.0,"1 จาน 150 ก.","กับข้าว",2.1,undefined,undefined,undefined,0,414],   // 11201 ลาบเนื้อ, รวมผักสด
["ลาบเลือดอีสาน",148,23.5,11.2,1.8,"1 จาน 150 ก.","กับข้าว",1.7,undefined,undefined,undefined,0],   // 11202 ลาบเลือด, อีสาน
// --- จานเดียว ---
["ขนมจีนน้ำยากะทิ",188,8.0,24.2,8.3,"1 จาน 250 ก.","จานเดียว",7.8,795,undefined,undefined,0,331],   // 11161 ขนมจีนน้ายากะทิ
["ขนมจีนน้ำพริก (รวมผัก)",380,8.5,49.5,16.5,"1 จาน 250 ก.","จานเดียว",0.8,undefined,undefined,undefined,0],   // 11142 ขนมจีนน้าพริก, รวมผัก
...
["ป๊อปคอร์นโรงหนัง รสเค็ม/เนย (ถ้วยกลาง)",450,8.1,51.5,25.3,"ถ้วยกลาง ~90 ก.","ของหวาน",9.0,796,4.4,0,0],   // ประมาณ: 170247 × 90 ก.
["ป๊อปคอร์นโรงหนัง รสคาราเมล (ถ้วยกลาง)",388,3.4,71.2,11.5,"ถ้วยกลาง ~90 ก.","ของหวาน",4.7,185,3.2,47.9,0],   // ประมาณ: 167552 × 90 ก.
...
["เนื้อสไลด์ชาบู/สุกี้ (ติดมัน) ดิบ",250,18.0,0,20.0,"100 ก.","วัตถุดิบ",0,60,8.0,0,0],   // ประมาณจากเนื้อสันคอติดมัน
["เนื้อวากิวสไลด์ ดิบ",400,14.0,0,38.0,"100 ก.","วัตถุดิบ",0,55,15.0,0,0],   // ประมาณ (มันแทรกสูง)
...
["สเต๊กเนื้อสันนอก 1 จาน (เนื้อ 200 ก.)",700,62.0,26.0,37.0,"1 จาน","จานเดียว",2.5,900,12.0,4.0,0],
["สเต๊กเนื้อ ร้านทั่วไป 1 จาน (เนื้อ 120 ก. + เฟรนช์ฟรายส์)",770,41.6,54.0,39.8,"1 จาน","จานเดียว",4.6,830,11.0,6.0,0],
["สเต๊กริบอาย 1 จาน (เนื้อ 250 ก.)",700,66.0,3.0,47.0,"1 จาน","จานเดียว",0.3,700,20.0,1.0,0],
["สเต๊กหมู 1 จาน (ร้านทั่วไป)",750,45.0,52.0,40.0,"1 จาน","จานเดียว",4.5,800,12.0,6.0,0],
["สเต๊กอกไก่ 1 จาน (ไก่ 180 ก.)",650,52.0,45.0,29.0,"1 จาน","จานเดียว",4.5,850,7.5,5.0,0],
["สเต๊กปลาแซลมอน 1 จาน",630,42.0,35.0,34.0,"1 จาน","จานเดียว",3.5,750,8.0,4.0,0],

// ===== เมนูเนื้อไทย/ตุ๋น (คำนวณจากส่วนผสม — ค่าประมาณ) =====
["เนื้อแดดเดียวทอด 100 ก.",350,37.6,3.0,20.8,"100 ก.","วัตถุดิบ",0,1100,9.0,3.0,0],
["เนื้อแดดเดียวทอด 1 จาน",245,26.3,2.1,14.6,"1 จาน ~70 ก.","กับข้าว",0,770,6.3,2.1,0],
["หมูแดดเดียวทอด 100 ก.",360,37.5,3.0,22.0,"100 ก.","วัตถุดิบ",0,1100,8.5,3.0,0],
["เนื้อสวรรค์",370,35.0,10.0,20.0,"100 ก.","วัตถุดิบ",0,900,8.0,9.0,0],
["ต้มเนื้อเปื่อย 1 ถ้วย",210,27.3,2.0,9.2,"1 ถ้วย 250 ก.","กับข้าว",0.4,700,3.3,1.0,0],
["ต้มแซ่บเนื้อเปื่อย 1 ถ้วย",200,25.0,4.0,9.0,"1 ถ้วย 250 ก.","กับข้าว",0.8,850,3.3,1.0,0],
["ก๋วยเตี๋ยวเนื้อตุ๋น 1 ชาม",380,27.5,34.0,11.6,"1 ชาม","ก๋วยเตี๋ยว",1.8,1950,4.5,3.0,0],
["แกงมัสมั่นเนื้อ 1 ถ้วย",480,30.0,19.0,30.5,"1 ถ้วย","กับข้าว",2.0,1000,18.0,6.0,0],
["พะโล้เนื้อ 1 ถ้วย",290,33.0,10.0,12.5,"1 ถ้วย","กับข้าว",0.2,1000,4.5,8.0,0],
["สตูว์เนื้อ 1 ถ้วย",350,29.0,25.0,14.0,"1 ถ้วย","กับข้าว",2.5,750,5.0,4.0,0],
["ซี่โครงวัวตุ๋นซอสบาร์บีคิว 1 ชิ้น",430,34.0,14.0,26.5,"1 ชิ้น ~150 ก.","กับข้าว",0.3,570,12.0,12.0,0],
["เนื้อย่างจิ้มแจ่ว 1 จาน",360,44.5,7.0,14.5,"1 จาน (เนื้อ 150 ก.)","กับข้าว",0.5,850,5.5,3.0,0],
["เนื้อผัดน้ำมันหอย 1 จาน",345,28.0,9.0,21.0,"1 จาน","กับข้าว",2.0,1300,6.0,2.0,0],
["ยำเนื้อย่าง 1 จาน",250,29.0,9.0,9.0,"1 จาน","กับข้าว",1.8,850,3.5,4.0,0],
["เนื้อย่างเกาหลี (บุลโกกิ) 1 จาน",490,38.0,17.0,28.0,"1 จาน","กับข้าว",0.8,900,9.0,13.0,0],

// ===== ปิ้งย่างเป็นไม้/เป็นชิ้น (คำนวณจากส่วนผสม — ค่าประมาณ) =====
["หมูปิ้งไม้ใหญ่",165,12.5,6.0,9.5,"1 ไม้ ~60 ก.","กับข้าว",0,480,4.2,5.5,0],
["หมูปิ้งติดมัน (คอหมู) 1 ไม้",130,7.5,3.5,9.5,"1 ไม้","กับข้าว",0,280,3.5,3.3,0],
["คอหมูย่าง 1 จาน",415,35.0,4.0,28.5,"1 จาน (หมู 150 ก.)","กับข้าว",0.2,600,10.5,3.0,0],
["หมูสามชั้นย่างเกาหลี 1 จาน",570,27.0,0,51.0,"1 จาน (หมู 150 ก.)","กับข้าว",0,80,18.5,0,0],
["อกไก่ปิ้ง 1 ชิ้น",290,46.5,1.0,10.4,"1 ชิ้น ~150 ก.","กับข้าว",0,400,2.5,0.5,0],
["อกไก่ปิ้ง 1 ไม้",70,12.0,1.0,2.0,"1 ไม้ ~40 ก.","กับข้าว",0,150,0.6,0.5,0],
["สะโพกไก่ย่าง 1 ชิ้น",280,28.8,1.0,17.5,"1 ชิ้น ~120 ก.","กับข้าว",0,300,4.8,0.5,0],
// ===== เครื่องปรุง/ซอส/น้ำจิ้ม =====
["ซีอิ๊วดำ",25,0.5,5.5,0,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0,1200,0,4,0],
["ซอสปรุงรส (ฝาเขียว)",10,1.3,1.0,0,"1 ช้อนโต๊ะ 16 ก.","วัตถุดิบ",0,1100,0,0.3,0],
["เต้าเจี้ยว",20,1.5,2.5,0.5,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0.3,1300,0.1,0.5,0],
["กะปิ",8,1.2,0.5,0.1,"1 ช้อนชา 6 ก.","วัตถุดิบ",0,900,0,0,0],
["พริกน้ำปลา",7,0.9,0.7,0,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0.1,1300,0,0.2,0],
["น้ำจิ้มแจ่ว",15,0.4,3.0,0.1,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0.2,450,0,1.5,0],
["น้ำจิ้มสุกี้",22,0.3,5.0,0.1,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0.2,350,0,3.5,0],
["น้ำจิ้มซีฟู้ด",12,0.3,2.5,0.1,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0.2,400,0,1.8,0],
["น้ำยำ (น้ำปรุงรสยำ)",18,0.3,4.0,0.1,"1 ช้อนโต๊ะ 18 ก.","วัตถุดิบ",0.1,500,0,3,0],
["ซอสพริก",15,0.2,3.5,0,"1 ช้อนโต๊ะ 17 ก.","วัตถุดิบ",0.2,450,0,3,0],
...
["น้ำสลัดงาญี่ปุ่น",60,0.6,3.0,5.2,"1 ช้อนโต๊ะ 15 ก.","วัตถุดิบ",0.1,180,0.8,2.5,0],
["มายองเนสไขมันต่ำ",36,0.1,1.4,3.3,"1 ช้อนโต๊ะ 15 ก.","วัตถุดิบ",0,124,0.5,0.5,0,5],   // USDA 173594
["น้ำตาลปี๊บ",76,0,19.0,0,"1 ช้อนโต๊ะ 20 ก.","วัตถุดิบ",0,10,0,19,0],
...
["ผงปรุงรส (ซุปผง)",10,0.5,2.0,0.1,"1 ช้อนชา 5 ก.","วัตถุดิบ",0,900,0,0.3,0],
["ซุปก้อน",25,1.0,2.5,1.2,"1 ก้อน 10 ก.","วัตถุดิบ",0,2000,0.6,0.3,0],
["พริกไทยดำป่น",6,0.2,1.5,0.1,"1 ช้อนชา 2.3 ก.","วัตถุดิบ",0.6,0,0,0,0,32],   // USDA 170931
["ผงขมิ้น",9,0.2,1.9,0.1,"1 ช้อนชา 3 ก.","วัตถุดิบ",0.6,1,0,0,0],
["น้ำมะนาว",4,0.1,1.3,0,"1 ช้อนโต๊ะ 15 ก.","วัตถุดิบ",0.1,0,0,0.3,0,19],   // USDA 168156
["น้ำมะขามเปียก",20,0.2,5.0,0.1,"1 ช้อนโต๊ะ 20 ก.","วัตถุดิบ",0.5,3,0,0,0],
["มะขามเปียก 100 ก.",239,2.8,62.5,0.6,"100 ก.","วัตถุดิบ",5.1,28,0.3,0,0,628],   // USDA 167763

// ===== เครื่องดื่ม =====
["กาแฟเอสเพรสโซ 1 ช็อต",3,0.1,0,0,"1 ช็อต 30 มล.","เครื่องดื่ม",0,1,0,0,0],
["กาแฟอเมริกาโน่ (ไม่ใส่น้ำตาล)",5,0.2,0,0,"1 แก้ว","เครื่องดื่ม",0,5,0,0,0],
["กาแฟคาปูชิโน่ (ไม่ใส่น้ำตาล)",75,4.0,6.0,4.0,"1 แก้ว 240 มล.","เครื่องดื่ม",0,55,2.4,0,0],
["กาแฟลาเต้เย็น (ไม่ใส่น้ำตาล)",112,6.0,9.0,6.0,"1 แก้ว","เครื่องดื่ม",0,80,3.6,0,0],
["กาแฟมอคค่าเย็น",320,8.0,45.0,12.0,"1 แก้ว","เครื่องดื่ม",1.5,130,7.0,34,0],
["ชาเขียวเย็น (หวานปกติ)",200,1.5,45.0,2.0,"1 แก้ว","เครื่องดื่ม",0,60,1.2,42,0],
["ชาดำเย็น (หวาน)",180,0,45.0,0,"1 แก้ว","เครื่องดื่ม",0,20,0,44,0],
["ชามะนาว (หวาน)",150,0,38.0,0,"1 แก้ว","เครื่องดื่ม",0,15,0,36,0],
["โกโก้ร้อน (ใส่นม+น้ำตาล)",220,8.0,30.0,8.0,"1 แก้ว","เครื่องดื่ม",2.0,130,4.5,22,0],
["ไมโล/โอวัลติน 1 แก้ว",200,7.0,30.0,5.5,"1 แก้ว","เครื่องดื่ม",0.6,150,3.2,20,0],
["น้ำส้มคั้นสด",108,1.7,25.0,0.5,"1 แก้ว 240 มล.","เครื่องดื่ม",0.5,2,0.1,20.2,0,480],   // USDA 169098 (WHO นับน้ำผลไม้เป็นน้ำตาลอิสระ)
["น้ำแอปเปิ้ล 1 กล่อง",92,0.2,22.8,0.2,"1 กล่อง 200 มล.","เครื่องดื่ม",0.4,5,0,21,0],
["น้ำอ้อย",190,0.2,47.0,0.2,"1 แก้ว 250 มล.","เครื่องดื่ม",0,30,0,45,0],
["น้ำใบเตย/น้ำเก๊กฮวย (หวาน)",110,0,27.5,0,"1 แก้ว 250 มล.","เครื่องดื่ม",0,15,0,27,0],
["น้ำกระเจี๊ยบ (หวาน)",100,0,25.0,0,"1 แก้ว 250 มล.","เครื่องดื่ม",0,10,0,24,0],
["น้ำมะนาวโซดา (หวาน)",120,0,30.0,0,"1 แก้ว","เครื่องดื่ม",0,20,0,29,0],
["น้ำขิง (หวาน)",90,0,22.0,0,"1 แก้ว","เครื่องดื่ม",0,10,0,21,0],
...
["ครีมเทียมผง",16,0.1,1.7,1.0,"1 ช้อนชา 3 ก.","วัตถุดิบ",0,6,0.9,1.5,0],
...
["เหล้าปั่น 1 แก้ว",320,0.5,45.0,0,"1 แก้ว","เครื่องดื่ม",0.3,25,0,42,14],
...
["โรตีกล้วยไข่ 1 ชิ้น",400,6.0,48.0,20.0,"1 ชิ้น","ของหวาน",1.5,300,10.0,22,0],
["ขนมโตเกียว 1 ชิ้น",90,2.0,13.0,3.2,"1 ชิ้น 35 ก.","ของหวาน",0.2,90,1.4,6,0],
["ขนมปังสังขยา 1 ชิ้น",250,5.0,35.0,10.0,"1 ชิ้น","ของหวาน",1.0,250,5.5,14,0],

// ===== เบเกอรี่ =====
["ขนมปังขาว 1 แผ่น",75,2.3,14.0,1.0,"1 แผ่น 28 ก.","วัตถุดิบ",0.7,137,0.2,1.2,0],
...
["ซาลาเปาไส้สังขยา 1 ลูก",210,5.0,35.0,5.5,"1 ลูก","ของหวาน",1.0,200,3.0,14,0],
...
["หมูยอ",220,13.0,6.0,16.0,"100 ก.","วัตถุดิบ",0,900,6.0,1,0],
...
["ปูอัด 1 ชิ้น",16,1.5,2.0,0.1,"1 ชิ้น 17 ก.","วัตถุดิบ",0,90,0,0.7,0],
...
["ไข่เค็ม 1 ฟอง",110,7.0,1.0,9.0,"1 ฟอง 60 ก.","วัตถุดิบ",0,800,3.0,0,0],
["ไข่เยี่ยวม้า 1 ฟอง",95,8.0,1.5,6.5,"1 ฟอง 60 ก.","วัตถุดิบ",0,500,2.0,0,0],
...
["ตะไคร้สด",99,1.8,25.3,0.5,"100 ก.","วัตถุดิบ",0,6,0.1,0,0],
...
["มะนาว 1 ลูก",11,0.3,3.7,0.1,"1 ลูก 44 ก.","วัตถุดิบ",0.2,1,0,0,0],
["กระเทียมดอง 1 หัว",12,0.3,2.7,0,"1 หัว 10 ก.","วัตถุดิบ",0.2,200,0,1.5,0],
["ผักกาดดอง",20,1.0,3.5,0.2,"100 ก.","วัตถุดิบ",1.5,1200,0,0.5,0],
["แตงกวาดอง",18,0.5,4.0,0.2,"100 ก.","วัตถุดิบ",1.2,800,0,2,0],
...
["ซีเรียลคอร์นเฟลก 1 ถ้วย",100,2.0,24.0,0.1,"1 ถ้วย 28 ก.","วัตถุดิบ",0.7,200,0,3,0],
["ข้าวโอ๊ตสำเร็จรูป 1 ซอง",120,4.0,21.0,2.5,"1 ซอง 32 ก.","วัตถุดิบ",3.0,180,0.5,6,0],
["แป้งข้าวเจ้า 1 ช้อนโต๊ะ",29,0.5,6.4,0.1,"1 ช้อนโต๊ะ 8 ก.","วัตถุดิบ",0.2,0,0,0,0],
["บะหมี่กึ่งสำเร็จรูป 1 ซอง (ดิบ)",280,6.0,40.0,11.0,"1 ซอง 60 ก.","วัตถุดิบ",1.8,1400,5.5,2,0],
["เส้นบุก (ชิราทากิ)",8,0.2,3.0,0,"100 ก.","วัตถุดิบ",2.9,10,0,0,0],
["ข้าวญี่ปุ่นหุงสุก 1 ทัพพี",130,2.4,28.7,0.2,"1 ทัพพี 100 ก.","วัตถุดิบ",0.3,1,0,0,0],
["แผ่นเกี๊ยว 5 แผ่น",70,2.3,14.0,0.3,"5 แผ่น 25 ก.","วัตถุดิบ",0.5,110,0,0,0],
["กาแฟโบราณ/โอเลี้ยง (หวาน)",180,0.5,42.0,1.5,"1 แก้ว","เครื่องดื่ม",0,40,1.0,40,0],
...
const SOUPRE=/ก๋วยเตี๋ยว|ก๋วยจั๊บ|บะหมี่|เย็นตาโฟ|ราเมง|เกาเหลา|สุกี้น้ำ/;
...
function chooseFood(name){
  ...
  el("fUnit").textContent="1 หน่วย = "+f[5]+(per?` (≈ ${per} ${/มล\.|ml/i.test(f[5])?"มล.":"ก."})`:"");
...
el("fAdd").onclick=async()=>{
  ...
  if(bf!==1) r.broth=bf;
  S.food.push(r);S.chosen=null;el("fChosen").style.display="none";
...
el("mCancel").onclick=()=>{el("fManual").style.display="none";el("fSearch").value="";};
...
el("rSave").onclick=async()=>{
  ...
  const unit=el("rUnit").value.trim()||(sv!==1?`1 มื้อ (สูตรทำ ${n1(sv)} มื้อ)`:"1 จาน (สูตรรวม)");
...
function drawBodyCharts(){
  ...
  if(arr.length){
    ...
    mk("c7",{data:{labels:lbl,datasets:[
  ...
  if(m.length){
    ...
    mk("c8",{type:"line",data:{labels:lbl,datasets:ds},options:{}});
  }
}
...
function dayData(d){
  ...
  const load=ex.reduce((a,b)=>a+(+b.load|| (+b.min||0)*2 ),0)
           + wo.reduce((a,b)=>a+(b.sets?b.sets.length*2.5:0),0);
...
function readiness(){
  ...
  /* 5) ความหนักเมื่อวาน — ถ้ายังไม่เคยบันทึกการออกกำลังเลย ไม่ใช่ว่า "เบา" แต่คือ "ไม่รู้" */
  const y=dayData(shiftDate(S.date,-1));
  const everEx = arr(S.ex).length>0 || arr(S.wo).length>0;
  const sY = !everEx? null : y.load>110? 45 : y.load>80? 70 : 100;
  f.push({k:"ความหนักเมื่อวาน",v:sY,w:.10,
    txt: !everEx? "ยังไม่เคยบันทึกการออกกำลังกาย"
       : y.load>110?`เมื่อวานหนักมาก (โหลด ${Math.round(y.load)}) ควรผ่อน`
       : y.load>80?`เมื่อวานหนักพอควร (${Math.round(y.load)})`:`เมื่อวานเบา ฟื้นตัวได้`});
...
function adaptiveTDEE(){
  ...
  if(fd.length<8) return {need:`บันทึกอาหารให้ครบอย่างน้อย 8 วันใน 21 วัน (ตอนนี้ ${fd.length} วัน)`};
  const w=S.body.filter(x=>x.w>0 && x.date>=days[0].key).sort((a,b)=>a.date<b.date?-1:1);
  if(w.length<2) return {need:"ชั่งน้ำหนักอย่างน้อย 2 ครั้ง ห่างกัน 7 วันขึ้นไป"};
  const span=dayDiff(w[0].date,w[w.length-1].date);
  if(span<7) return {need:"ชั่งน้ำหนักให้ห่างกันอย่างน้อย 7 วัน"};
  const dW=w[w.length-1].w-w[0].w;
  const avgIn=Math.round(fd.reduce((a,b)=>a+b.kIn,0)/fd.length);
  const avgOut=Math.round(days.reduce((a,b)=>a+b.kOut,0)/days.length);
  const real=Math.round(avgIn - (dW*7700/span));      // TDEE จริง = กินเฉลี่ย − พลังงานที่หายไปกับน้ำหนัก
  return {real,avgIn,avgOut,dW:n1(dW),span,days:fd.length,formula:S.user.tdee};
}
function applyRealTdee(){
  const t=adaptiveTDEE(); if(!t||!t.real)return;
...
function render(){
  ...
  setHTML(el("secFood"),`
    ...
    ${d.fd.length? subFold("homeMeals",`🍽️ ที่กินวันนี้ <span class="pill ok">${d.fd.length} รายการ</span>`, sortByTime(d.fd).map(x=>
      `<div class="meal"><div>${x.time?`<b style="color:var(--food)">${x.time}</b> `:""}${mealIcon(x.meal)} ${esc(x.name)}
        <small style="display:block;color:var(--dim);font-size:11px">${x.qty} × ${esc(x.unit)} · P${n1(x.protein)} C${n1(x.carb)} F${n1(x.fat)}${x.sodium>=700?` · <span style="color:var(--bad)">🧂 ${x.sodium}</span>`:x.sodium?` · 🧂 ${x.sodium}`:""}</small></div>
  ...
  setHTML(el("secMove"),`
    ...
    <div class="mini" style="margin-top:6px">คาร์ดิโอ ${Math.round(cLoad)} + เวท ${wLoad} · โหลด = เวลา × ความหนัก</div>
  ...
  {const t=adaptiveTDEE();
   if(t && t.real){
     ...
     el("tdeeCard").style.display="block";
     el("secTdee").innerHTML=`<div class="mini">ระบบจะคำนวณ TDEE จริงของคุณจากน้ำหนักที่เปลี่ยนไปเทียบกับที่กิน — ${t.need}</div>`;
  ...
  el("fList").innerHTML=d.fd.length?sortByTime(d.fd).map(x=>{
    ...
    return row(
      ...
      `${x.qty} × ${esc(x.unit)} · P${n1(x.protein)} C${n1(x.carb)} F${n1(x.fat)}
       <span class="nchs">${
         chip("🥦","ใย",`${e("fiber")}${n1(nr.fiber)} ก.`)+
         `<span class="nch"${sodiumHot?' style="color:var(--bad)"':''}>🧂 ${e("sodium")}${Math.round(nr.sodium).toLocaleString()} มก.</span>`+
         chip("🧈","อิ่มตัว",`${e("sat")}${n1(nr.sat)} ก.`)+
      ...
      `<span style="color:var(--food)">${x.kcal}</span>`,"food",x.ts);
  }).join(""):`<div class="empty">ยังไม่มีบันทึก</div>`;
  el("fListNote").innerHTML = (d.fd.length&&anyEst)
    ? `<div class="mini">~ = ค่าประมาณจากชนิดอาหาร (เมนูนั้นยังไม่มีค่าจริงในฐานข้อมูล) · ตัวเลขที่ไม่มี ~ คือค่าจริง</div>` : "";
...
function row(title,sub,val,kind,ts){
...
function drawCharts(){
  ...
  ].map(k=>`<div class="k"><span>${k[0]}</span><b style="color:${k[3]}">${k[1]}</b>${k[2]}</div>`).join("");
...
function mk(id,cfg){
  ...
  const Y=(v,m)=>padT+ih-(v/(m||max))*ih;

  let g="",yl="";
  for(let k=0;k<=4;k++){
    const y=padT+ih-k*ih/4, v=max*k/4;
  ...
  ds.filter(d=>(d.type||cfg.type)!=="bar").forEach(d=>{
    const vals=num(d.data), m=d.yAxisID==="y1"?max1:max, useBar=bars.length>0;
